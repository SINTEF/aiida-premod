...
class PreModCalculation(CalcJob):
    ...
    def define(cls, spec):
        ...
        spec.output('log', valid_type=SinglefileData, help='The log generated by premod.')
        spec.output('summary', valid_type=SinglefileData, help='A summary of the results.')
        spec.output('history', valid_type=SinglefileData, help='A history.')
        spec.output('microstructure', valid_type=SinglefileData, help='The microstructure generated from the calculation.')

        spec.exit_code(100, 'ERROR_MISSING_INPUT_FILES', message='Expected input files was not supplied.')
        spec.exit_code(101, 'ERROR_MISSING_OUTPUT_FILES', message='Calculation did not produce all expected output files.')
        spec.exit_code(102, 'ERROR_NO_RETRIEVED_FOLDER', message='Retrieved folder does not exist.')
